...
foreach($selfdata['children'] as $key => $value) {
	...
	$r =  $this->Html->div('col-md-3 col-sm-3 col-xs-3 no-padding', $image);
	
	if(isset($value['children'])) {
		foreach($value['children'] as $k => $l) {
			
			$heading[$l['Configuration']['title']] = $this->Html->div('col-md-3 col-sm-3 col-xs-3 no-padding', 
			...
			$r .= $this->Html->div('col-md-3 col-sm-3 col-xs-3 droppable-answer', $answer, array(
...
if(!$summary) {
	...
	$discard = $this->Html->div('col-md-6 col-sm-6 col-xs-6 droppable-discard', 'Drop Here to Discard');
	$wildCrd = $this->Html->div('col-md-6 col-sm-6 col-xs-6 add-wild-card', 
	...
	$final = $this->Html->div('col-md-8 col-sm-8 col-xs-12 no-padding drop-here-box', $imageRow . $actions) .
			 $this->Html->div('col-md-4 col-sm-4 col-xs-4 no-padding padding-left-10', $ul);
