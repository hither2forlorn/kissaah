...
foreach($selfdata['children'] as $child) {
	...
	if(isset($child['Dependent'])) {
		...
		foreach($child['Dependent'] as $dependent) {
			...
			$challenge = $this->Html->div('col-md-12 btn-in-progress text-left margin-bottom-5', 
								(empty($goal['Challenge']['name']))? $dependent['answer']: $goal['Challenge']['name']);
			$complete_by = $this->Html->div('col-md-4 btn-in-progress text-left ', 'Created: ' . date('m/d/Y', strtotime($dependent['created'])));
	
			$options['data-depn'] 	= $dependent['id'];
			$options['class'] 		= 'form-control date-picker-future';
			$options['div'] 		= 'col-md-4';
			$options['type'] 		= 'text';
			...
			if(!$summary) {
				...
				$complete_by .= $this->Html->div('col-md-4', $this->Form->label('Challenge.complete_by', $options['value'], array('class' => 'control-label')));
			}
			...
			$calendar = $this->Html->link('Add to Calendar' . $calendar, '#', array('class' => 'pull-right addthisevent event', 
																					...
																					'escape'=> false));
			$calendar = $this->Html->div('col-md-4 no-padding', $calendar);
			...
			if($summary) {
				...
				$left_block = $this->Html->div('col-md-10 padding-right-0', $challenge_id . $challenge . $complete_by . 
																			$modalbtn . $description . $modal);
				$save_challenge = ' modal-challenge';
				
			} else {
				$left_block = $this->Html->div('col-md-10 padding-right-0', $challenge . $complete_by . $calendar . $description . 
			...
			$ally = $this->Html->div('col-md-12 btn-in-progress margin-bottom-5', $ally, array('data' => 'label-' . $dependent['id']));
			...
			$rght_block = $this->Html->div('col-md-2 no-padding', $ally . $img . $name);
