 ...
 *	'rel' => the array key to act as rel for a simple ul (Modified by HT : HimalayanTechies : HTSolutions)
 ...
 */
	function generate ($data, $settings = array ()) {
		$this->__settings = array_merge(array(
				'rel' => null, //Added by HT : HimalayanTechies : HTSolutions
				'model' => null,
				'alias' => 'title',
				'type' => 'ul',
				'itemType' => 'li',
		...
		foreach ($data as $i => $result) {
			...
			$return .= '<p id="'.$result[$model]['id'].'" class="tree"';
			if(!empty($rel)) $return .= ' rel="'.$result[$model][$rel].'" ';
			$return .= '>'.$content.'</p>';
			/* Suffix */
			$__addType = false;
